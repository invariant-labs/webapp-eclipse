...
export const BuyComponent: React.FC<IProps> = ({ alertBoxText, raisedAmount, totalAmount, onBuyClick }) => {
    const { classes } = useStyles({ percentage: (+raisedAmount / +totalAmount) * 100 })
    const [alertBoxShow, setAlertBoxShow] = useState(true)
    const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<PaymentMethod>(PaymentMethod.VIRTUAL_CARD)
    return (
        <Box className={classes.container}>
            {alertBoxText && alertBoxShow && (
            ...
            <Box className={classes.sectionDivider}>
                ...
                <Box className={classes.paymentOptions}>
                    <Box className={classNames(classes.paymentOption, selectedPaymentMethod === PaymentMethod.VIRTUAL_CARD ? classes.paymentSelected : '')} onClick={() => setSelectedPaymentMethod(PaymentMethod.VIRTUAL_CARD)}>
                    ...
                    <Box className={classNames(classes.paymentOption, selectedPaymentMethod === PaymentMethod.CRYPTO_USDC ? classes.paymentSelected : '')} onClick={() => setSelectedPaymentMethod(PaymentMethod.CRYPTO_USDC)}>
            ...
            <Box>
                ...
                <Box className={classes.youllReceiveBox}>
            ...
            <Button scheme='green' padding='0 42px' height='44px' onClick={onBuyClick}>
                Buy $INV
            </Button>
        </Box>
    )
