...
const faqData = [
  {
    ...
    answer: (
      <Typography sx={{ zIndex: 5, position: 'relative' }}>
        ...
        position and automatically increases in value as staking rewards accumulate. Instead of
        claiming rewards manually once or twice a day, Invariant introduced an auto-compounding
        mechanism that compounds staking rewards every ten seconds. This increases the base amount
        from which rewards are calculated, allowing you to achieve a significantly higher APY
        compared to traditional BITZ staking. In addition, you can use sBITZ in DeFi protocols (for
        example, by providing liquidity) to earn extra yield while still continuously earning
        staking rewards.
      </Typography>
    )
  },
...
For Eclipse DeFi, it’s a solution to the liquidity problem for BITZ.`
  },
  {
    ...
    answer:
      'You can get sBITZ by staking your BITZ using the interface above or simply by swapping BITZ → sBITZ.'
  },
  {
...
For example, if 1 sBITZ equals 1 BITZ at the start, it might be worth 1.05 BITZ after some time. The yield compounds automatically and no action is required.`
  },
  {
    ...
    answer:
      'The same way as with staking — either through the interface above or by swapping sBITZ to BITZ, or even to ETH.'
  },
  {
  ...
  },
  {
    ...
    answer:
      'The same as holding BITZ. Staking through Invariant is no different from staking on PowPow, except that you receive sBITZ in return. The staked BITZ goes to the same smart contract as when staking via PowPow.'
  }
]
