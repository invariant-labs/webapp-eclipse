...
import { BASE_API_URL } from '@store/consts/static'
...
async function fetchLpLeaderboardData(
  ...
  const response = await fetch(
    `${BASE_API_URL}/api/eclipse-${network}/lp/${userWallet}?offset=${offset}&size=${itemsPerPage}`
  )
  if (!response.ok) {
...
async function fetchContentPoints(userWallet?: string) {
  const response = await fetch(`${BASE_API_URL}/api/content-program/${userWallet}`)
  if (!response.ok) {
...
async function fetchSwapLeaderboardData(
  ...
  const offset = (page - 1) * itemsPerPage
  const response = await fetch(
    `${BASE_API_URL}/api/eclipse-${network}/swaps/${userWallet}?offset=${offset}&size=${itemsPerPage}`
  )
  if (!response.ok) {
    throw new Error('Failed to fetch leaderboard data')
  }
...
async function fetchTotalLeaderboardData(
  ...
  const offset = (page - 1) * itemsPerPage
  const response = await fetch(
    `${BASE_API_URL}/api/eclipse-${network}/total/${userWallet}?offset=${offset}&size=${itemsPerPage}`
  )
  if (!response.ok) {
    throw new Error('Failed to fetch leaderboard data')
  }
...
async function fetchLeaderboardPriceFeed() {
  const response = await fetch(`${BASE_API_URL}/api/swap/price-feeds`)
  if (!response.ok) {
    throw new Error('Failed to fetch leaderboard data')
  }
